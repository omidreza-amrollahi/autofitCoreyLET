...
def get_let_params(grouped_data):
    let_params = {}
    param_bounds_let = [(0.1, 0, 0.5), (20, 20, 5)]
    for i, (well_id, well_data) in enumerate(grouped_data):
        ...
        if len(Sw)>2:
            try:
                popt, _ = curve_fit(lambda s, lw, ew, tw: let(s, krw_sor, lw, ew, tw), Sw, krw, bounds=param_bounds_let)
                lw, ew, tw = popt
